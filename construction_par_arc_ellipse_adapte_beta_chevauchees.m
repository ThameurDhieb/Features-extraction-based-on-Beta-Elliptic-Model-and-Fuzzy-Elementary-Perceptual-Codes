function  [param_trajectoire, param_grandeur, e_indice, Vect_Erreurs, Sensibilite_plus, DAT] = construction_par_arc_ellipse_adapte_beta_chevauchees(fonctions_beta, tableau_affectation_intervalles, vect_X0XCX1, points, T, num_fig)
...
for i = 1 : nbr_intervalles
    ...
    if ( (indiceM2 - indiceTemps_deb + 1 - uu) > 0 ) & ( (indiceM2 - indiceTemps_deb + 1 + uu) < L_DAT )
       ...
       teta_M2 = DAT(indiceM2 - indiceTemps_deb + 1);
    end
...
%%%^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^%%%
       ...
       autorisation_de_representation_graphique_de_l_arc = 0; %1;
